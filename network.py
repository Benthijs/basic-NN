...
class Network:
    ...
    def append(self, N, activation):
        ...
        if(self.layers.shape[0] != 0):
            lay = layer.Layer(N, activation, self.layers[0].Neurons.shape[0])
    ...
    def forward(self, synapses):
        ...
        for j in range(self.layers.shape[0]):
            ...
            if j+1 < self.layers.shape[0]: new_synapses = np.array([list(new_synapses)] * self.layers[j].Neurons.shape[0]) 
